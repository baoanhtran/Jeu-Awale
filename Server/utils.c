...
const char* extract_first_word(const char* str, char* result, int max_len) {
	while (*str == ' ') {str++;} //Trim spaces in beginning
...
void trim_whitespace(char *str) {
    char *end;
    while (isspace((unsigned char)*str)) str++; // skip whitespace in the beginning
    if (*str == '\0') return;
    end = str + strlen(str) - 1;
    while (end > str && isspace((unsigned char)*end)) end--; // skip whitespace at the end
    *(end + 1) = '\0';
}
