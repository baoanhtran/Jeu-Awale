...
void getClients(Client *clients, int *nb_clients)
{
    ...
    while (!feof(file))
    {
        ...
        if (file_read_client(&clients[len], file) == 0)
            break;
...
void updateClient(Client newClient)
{
    ...
    {
        Client client;
        if (file_read_client(&client, file) == 0)
            break;
...
size_t file_read_client(Client *client, FILE *file)
{
    ...
    len += t;
    if ((t = fread(&client->elo_scores, sizeof(int), 1, file)) != 1)
    {
        return 0;
    }
    len += t;

    if ((t = fread_string(client->name, file)) == 0)
    {
        return 0;
    }
    len += t;
    if ((t = fread_string(client->bio, file)) == 0)
    {
        return 0;
    }
    len += t;
    if ((t = fread_string(client->ip, file)) == 0)
    {
        return 0;
    }
    len += t;
